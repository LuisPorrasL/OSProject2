...
int AddrSpace::updateTLBSC(unsigned int vpn)
{
	//Debo guardar los cambios hechos en la TLB a la pagina que estoy sacando.
	int returnIndex = 0;

	bool victimaFound = false;

	if(!threadFirstTime){ //Si no es la primera pasada

		while(!victimaFound){
			if(machine->tlb[indexTLBFIFO].use == false){ //Encontre a la pagina victima del tlb
				victimaFound = true;
				returnIndex = indexTLBFIFO;

				//Actualizo la pagina del pageTable de la victima antes de sacarla
				pageTable[machine->tlb[indexTLBFIFO].virtualPage].use = machine->tlb[indexTLBFIFO].use;
				pageTable[machine->tlb[indexTLBFIFO].virtualPage].dirty = machine->tlb[indexTLBFIFO].dirty;

				//Cargo la nueva pagina al TLB
				machine->tlb[indexTLBFIFO].virtualPage =  pageTable[vpn].virtualPage;
				machine->tlb[indexTLBFIFO].physicalPage = pageTable[vpn].physicalPage;
				machine->tlb[indexTLBFIFO].valid = pageTable[vpn].valid;
				machine->tlb[indexTLBFIFO].use = pageTable[vpn].use;
				...
				machine->tlb[indexTLBFIFO].use = false;
				pageTable[machine->tlb[indexTLBFIFO].virtualPage].use = machine->tlb[indexTLBFIFO].use;
				pageTable[machine->tlb[indexTLBFIFO].virtualPage].dirty = machine->tlb[indexTLBFIFO].dirty;
			}
			indexTLBFIFO = (indexTLBFIFO + 1)%TLBSize;
...
void AddrSpace::writeIntoSwap( int physicalPageVictim ){
	...
	MemBitMap->Clear( physicalPageVictim );
	//clearPhysicalPage( victimMemoryPage );
	++stats->numDiskWrites;
	delete swapFile;
}
...
int AddrSpace::findVictimInMemSC(){
	int returnIndex = 0;
	bool victimaFound = false;

	while(!victimaFound){
		if(IPT[indexSWAPFIFO]->use == false){ //Encontre a la pagina victima de memoria
			victimaFound = true;
			returnIndex = indexSWAPFIFO;
		}
		else{
			IPT[indexSWAPFIFO]->use = false;
		}
...
int AddrSpace::findVictimInMemFIFO(){
	int oldIndexFIFO = indexSWAPFIFO;
	indexSWAPFIFO = (indexSWAPFIFO+1)%NumPhysPages;
	return oldIndexFIFO;
}
...
void AddrSpace::load(unsigned int vpn)
{
	...
	if ( !pageTable[vpn].valid && !pageTable[vpn].dirty ){
		...
		if(vpn >= 0 && vpn < initData){ //segemento de Codigo
			...
			{		//victima swap
				int victimMemoryPage = findVictimInMemSC();
				int tlbIndexForNewPage = findVictimInTLB( victimMemoryPage );

				if ( IPT[victimMemoryPage]->dirty )
				{
					DEBUG('v',"\t\t\tVictima f=%d,l=%d y  sucia\n",IPT[victimMemoryPage]->physicalPage, IPT[victimMemoryPage]->virtualPage );
					writeIntoSwap( IPT[victimMemoryPage]->physicalPage );
				...
				}else
				{
					...
					int oldPhysicalPage = IPT[victimMemoryPage]->physicalPage;
					IPT[victimMemoryPage]->valid = false;
					IPT[victimMemoryPage]->physicalPage = -1;
